...
class ActForm(forms.ModelForm):
    ...
    class Meta:
        model = Acts
        fields = '__all__'
        field_order = ['act_date', 'avtor', 'inv_dit', 'new_user',
                       'result', 'conclusion', 'type', 'new_sklad']
        widgets = {
            ...
            "inv_dit": ModelSelect2Widget(queryset=models.IT_OS.objects.all(),
                                          attrs={
                                              'class': 'form-field inv_dit'},
                                          search_fields=['inv_dit__icontains'],
                                          search_option='icontains',  # Добавьте это поле для поиска без учета регистра
                                          ),
            "new_sklad": ModelSelect2Widget(queryset=models.SkladyOffice.objects.all(),
                                            attrs={
                                                'class': 'form-field select'},
                                            search_fields=[
                                                'sklad_name__icontains', 'sklad_name_lower__icontains'],
                                            ),
            "new_user": ModelSelect2Widget(queryset=models.Users.objects.all(),
                                           attrs={'class': 'form-field text',
                                                  'id': 'id_user', 'name': 'user'},
                                           search_fields=[
                                               'name_lower__icontains', 'name__icontains']
                                           ),
        }


class AddOsForm(forms.ModelForm):
    int_inv_dit = forms.CharField(max_length=50, label='Инвентарный номер',
                                  required=True, widget=forms.TextInput(attrs={'class': 'form-field input'}))
    first_part = forms.CharField(max_length=50, label='Процессор', required=False,
                                 widget=forms.TextInput(attrs={'class': 'form-field input'}))
    second_part = forms.CharField(max_length=50, label='ОЗУ', required=False, widget=forms.TextInput(
        attrs={'class': 'form-field input'}))
    third_part = forms.CharField(max_length=50, label='Накопитель', required=False,
                                 widget=forms.TextInput(attrs={'class': 'form-field input'}))
    model = forms.CharField(max_length=150, label='Наименование', required=False,
                            widget=forms.TextInput(attrs={'class': 'form-field input'}))
    department = forms.CharField(max_length=150, label='Подразделение', required=True,
                                 widget=forms.TextInput(attrs={'class': 'form-field input'}))
    user = forms.CharField(max_length=150, label='Пользователь', required=True,
                           widget=forms.TextInput(attrs={'class': 'form-field input'}))
    inv_dit = forms.CharField(max_length=50, label='', required=False)
    inpute_date = forms.DateTimeField(label='Дата', required=False, widget=forms.DateInput(
        attrs={'class': 'form-field input', 'readonly': 'readonly'}))
    ...
    def clean(self):
        ...
        selected_group = next(
            (group for group in GROUP_CHOICES if group[1] == os_group), None)
